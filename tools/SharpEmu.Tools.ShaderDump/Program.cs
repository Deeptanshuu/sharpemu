...
// checked with spirv-val / spirv-dis, and executed against expected values
// with SharpEmu.Tools.GpuConformance.
//
...
(string Name, uint[] Words)[] testPrograms =
[
    ("fmac", [
...
foreach (var (name, words) in testPrograms)
{
    ...
    {
        Console.WriteLine($"[{name}] emit: decode failed ({decodeArgs[3]})");
        continue;
    }
